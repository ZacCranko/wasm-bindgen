...
pub struct WasmBindgenDescriptorsSection {
    ...
    /// metadata (name, kind, namespace, catch, ...); an empty `shim` marks a
    /// `wbg_cast` identity adapter, which needs no AST metadata.
...
impl WasmBindgenDescriptorsSection {
    ...
    /// its `(shim, concrete signature)`, and groups the originating function
    /// ids by that key. A non-empty `shim` carries the AST key so the CLI can
    /// later attach real JS binding metadata (name/kind/namespace/catch); an
    /// empty `shim` marks a cast, which the CLI turns into an identity adapter.
